...
// Lookup tables for Galois Field operations
...
fn div(a: u8, b: u8) -> u8 {
    ...
    let log_a = LOG_TABLE[a as usize];
    let log_b = LOG_TABLE[b as usize];
    let diff = (log_a as i16 - log_b as i16 + 255) % 255;
    EXP_TABLE[diff as usize]
}

fn eval_polynomial(coefficients: &[u8], x: u8, degree: usize) -> u8 {
    if x == 0 {
        panic!("Cannot evaluate polynomial at x=0");
    }
...
fn interpolate_polynomial(x_samples: &[u8], y_samples: &[u8], x: u8) -> u8 {
    ...
    for i in 0..limit {
        ...
        result = add(result, mult(y_samples[i], basis));
    }
    result
}
...
pub fn split(secret: &[u8], shares: usize, threshold: usize,rng: &mut (dyn RngCore + '_)) -> Result<Vec<Vec<u8>>, ShamirError> {
    ...
    for (i, &byte) in secret.iter().enumerate() {
        ...
        for j in 0..shares {
            ...
            let y = eval_polynomial(&coefficients, x, degree);
            result[j][i] = y;
        }
    }
...
pub fn combine(shares: &[Vec<u8>]) -> Result<Vec<u8>, ShamirError> {
    if shares.len() < 2 || shares.len() > 255 {
        return Err(ShamirError::InvalidShareCount);
    }
    ...
    for share in shares {
        ...
        if !seen.insert(share[share_len - 1]) {
    ...
    let mut x_samples = vec![0u8; shares_len];
    let mut y_samples = vec![0u8; shares_len];

    // Get x-coordinates from shares
    for i in 0..shares_len {
        x_samples[i] = shares[i][share_len - 1];
    }
    ...
    for i in 0..secret_len {
        ...
        secret[i] = interpolate_polynomial(&x_samples, &y_samples, 0);
    }
